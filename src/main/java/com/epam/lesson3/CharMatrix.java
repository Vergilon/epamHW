...
/**
 * Char matrix for creating strings according to determinated rules
 *
...
public class CharMatrix {
    ...
    /**
     * Constructor random latin letters matrix
     * @param size set size of matrix
     */
    public CharMatrix(int size) {
        Random r = new Random();
        charMatrix = new char[size][size];
        for (int i = 0; i < charMatrix.length; i++) {
            for (int j = 0; j < charMatrix.length; j++) {
                charMatrix[i][j] = (char) (r.nextInt(26) + 'a');
            }
        }
    ...
    /**
     * Display matrix in readable form
     */
    public void showCharMatrix() {
        for (int i = 0; i < charMatrix.length; i++) {
            for (int j = 0; j < charMatrix.length; j++) {
